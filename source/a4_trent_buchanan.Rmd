...
ggplot(data = juvenile_count, #I left in the 2002 and 2009 gaps because I think that's important to at least show. 0 data is just as important as data.
       aes(x = year, y = n)) +
  geom_col() +
  labs(x = "Year",
       y = "Count",
       title = "Annual juvenile hare trap counts",
       subtitle = "1999 - 2012", #Said 1999 here because it was the first instance of juvenile data. 1998 didn't have any.
       tag = "Figure 1",
  ...
  scale_x_continuous(breaks = seq(1999,2012,1)) + #I think each year being shown is appropriate
...
site_summary <- hares_juvenile %>% 
  group_by(sex, grid) %>% #this was key in fixing the means and standard deviations
  mutate(mean_weight = mean(weight, na.rm = T),
         sd_weight = sd(weight, na.rm = T))
  

grid_names <- c("Lowland", "Mature", "Riparian") #this was for renaming the grid titles, I sw this technique first, tried it, and it worked, so I didn't try fixing it in the plot code
names(grid_names) <- c("bonbs", "bonmat", "bonrip")

site_count <- hares_juvenile %>% #this was for supplementing analysis
  count(grid)
...
ggplot() +
  geom_beeswarm(data = hares_juvenile,
                ...
                size = 3,
                alpha = 0.7) +
  facet_wrap(~grid, labeller = labeller(grid = grid_names)) + #here is where I implemented the grid title fix
  ...
  geom_point(data = site_summary, aes(x = sex, y = mean_weight), 
             color = "black",
             size = 3) +
  facet_wrap(~grid, labeller = labeller(grid = grid_names)) + #Glad the facet wraps match up nicely
  geom_errorbar(data = site_summary, #I chose error bar over boxplot because I think it looks cleaner. The boxplot got skinny and squased due to the nature of three side-by-side facet wrapped graphs
                aes(x = sex, 
...
kbl(juvenile_summary) %>% #Kable styling table appears clean and looks alright, so  I did not try to mess with it
  kable_styling()

```
```{r include=FALSE}

ggplot(data = hares_juvenile, aes(x=weight)) + #this is all exploratory and simply contributes to the analysis. Not included in the report because it is unnecessary in terms of visualization
  geom_histogram()
...
cohen.d(juveniles_f, juveniles_m, na.rm = T) #ran into issues with this, but the na.rm = T was very helpful. I just wasn't sure where to put it at first.
...
foot_weight <- hares_juvenile %>% 
  select(weight, hindft) #Selecting for the two variables of interest just so I do not accidentally mess up the hares_juvenile df
...
ggplot(data = foot_weight, 
  ...
  scale_y_continuous(breaks = seq(0,1700,200)) + #I like these breaks, it makes it easier to align the data points, but I hope it doesn't make it too messy or crowded.
  theme_bw()
...
juvenile_lm <- lm(weight ~ hindft, data = foot_weight) #not included in the report because this is exploratory and helps with the analysis. Not necessary to see though.
...
juvenile_coef <- juvenile_lm_tidy$estimate[2] #lost track of this in the global environment, but glad I found it for my summary of findings

juvenile_lm_out <- broom::glance(juvenile_lm) #still not sure how broom works, but it produces a nicely accessible table for in-line referencing

plot(juvenile_lm) #interesting how this plots the four graphs. Kind of feels like I should include these, or at least a couple of them, but I will not. 
...
ggplot(data = foot_weight, 
  ...
  geom_smooth(method = "lm",
              color = "cyan2", #changed color to cyan as I thought it stood out more
              size = 1,
              fill = "gray10",
              alpha = 0.4) +
  labs(x = "Hind Foot Length (mm)",
       y = "Weight (g)",
       title = "Hind foot length vs. weight in juvenile hares with trend line", #not sure how to name this one, 'with trend line'? 'with linear regression'?
       tag = "Figure 4",
  ...
  theme_bw() +
  ggpubr::stat_regline_equation(label.x = 65, label.y = 1350) #figured I would include this
